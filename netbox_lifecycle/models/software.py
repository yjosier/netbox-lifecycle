...
__all__ = ('Software', 'SoftwareAssignment')


class Software(PrimaryModel):
    ...
    def get_absolute_url(self):
        return reverse('plugins:netbox_lifecycle:software', args=[self.pk])


class SoftwareAssignment(PrimaryModel):
    software = models.ForeignKey(
        to='netbox_lifecycle.Software',
        on_delete=models.CASCADE,
    ...
    def __str__(self):
        if self.device:
            return f'{self.device.name}: {self.software.name}'
        if self.virtual_machine:
            return f'{self.virtual_machine.name}: {self.software.name}'
        return f'{self.software.name}'

    def get_absolute_url(self):
        return reverse('plugins:netbox_lifecycle:softwareassignment', args=[self.pk])
    ...
    def assigned_object(self):
        """Return the device or virtual machine assigned to this software."""
